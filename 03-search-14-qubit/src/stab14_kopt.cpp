...
int main(int argc, char** argv) {
    ...
    // M1 = M2 = 0 means unlimited breadth, i.e. the 4-opt sweep really is exhaustive; only a
    // positive M truncates it.  Label the two cases correctly.
    if (did4) {
        if (M1 <= 0 && M2 <= 0)
            ap("  4-opt: BEST FOUND = %lld / %zu  (UNLIMITED breadth: every 4-dimensional\n"
               "         isotropic subspace of every K^perp/K over all 70 drop-quadruples --\n"
               "         the 4-opt neighbourhood is EXHAUSTED)\n", (long long)globalBest, U3.size());
        else
            ap("  4-opt: BEST FOUND = %lld / %zu  (breadth-limited M1=%d M2=%d, NOT exhaustive)\n",
