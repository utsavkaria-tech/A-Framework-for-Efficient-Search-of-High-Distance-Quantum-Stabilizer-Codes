...
# -*- coding: utf-8 -*-
"""
stabilizer_search_benchmark_cyclic.py
=====================================

EXPERIMENT (D): CYCLICITY RUNTIME BENCHMARK for [[5,1,3]]_2.
...
----------------------------------------
    row_{i+1} == cyclic_shift(row_i, shift)

exactly as already defined by CyclicConstraint in [S4] -- same definition,
same shift table, same convention.  The shift acts on qubit indices and is
applied independently to the X block and to the Z block.  --cyclic-shift
selects the shift (default 1).

Explicitly NOT imposed: rank(H) = n-k, H_X H_Z^T = H_Z H_X^T (mod 2), any
weight spectrum, distance = 3, any [[5,1,3]] validity test, or any further
automorphism condition.

ENUMERATION MODE
----------------
--mode generate (DEFAULT) enumerates every possible first row and
generates the remaining rows from it by repeated shifts, performing NO
cyclicity test afterwards: every matrix built is cyclic by construction
and every cyclic matrix is built exactly once, so the enumerated count
is S_R itself and the runtime scales with S_R.

--mode filter instead traverses all 2^40 candidates and tests each one.
That is methodologically identical to the other three benchmarks but its
runtime is floored by the cost of walking 2^20 blocks, which for a
constraint this selective dominates completely. State which mode a
reported number came from.

RETENTION
---------
The whole matrix is determined by its first row, so S_R = 2^2n = 1024 and
S_R/S_0 = 1024/2^40 = 9.3132e-10.  Because the constraint is this selective,
runtime is dominated by traversing the block structure (2^20 blocks) rather
than by candidate processing -- the report prints the block accounting.
The search never uses this figure; --dry-run prints it for validation only.
...
Exactly one block per value of row 0 survives, so any contiguous region is
uniformly representative for this constraint.
...
def build_plan(spec: CodeSpec, constraint: Constraint, mode: str, backend: str,
    ...
    elif isinstance(constraint, CyclicConstraint):
        # ---- "generate": a cyclic matrix is determined by its first row,
        # so enumerate every possible first row; the engine generates the
        # remaining rows by repeated shifts and performs NO cyclicity test.
        row_values = [full]
    else:
...
def search_numpy_cyclic(plan: SearchPlan) -> SearchResult:
    """
    CONSTRAINED ENGINE for the cyclicity constraint (EXPERIMENT D).

    The ONLY condition imposed is the cyclic structural condition already
    defined by CyclicConstraint in [S4] -- the same definition, the same
    shift table, the same convention, unchanged:

        row_{i+1} == cyclic_shift(row_i, shift)

    the shift acting on qubit indices and applied independently to the X
    block and to the Z block.  Nothing else is imposed: no rank(H) = n-k, no
    H_X H_Z^T = H_Z H_X^T, no weight spectrum, no distance, no further
    automorphism condition, no [[5,1,3]] validity test.
    ...
    in this set, so the runtimes are directly comparable.
      ...
      Stage 1  the outer rows must already form a shift chain.  If they do
               not, every candidate sharing that prefix violates the
               condition and the whole block is rejected.
      Stage 2  given the outer rows, BOTH inner rows are then forced:
                   r_{rows-2} = shift(last outer row)
                   r_{rows-1} = shift(r_{rows-2})
               so exactly one candidate per surviving block is accepted.

    This constraint is extremely selective (S_R = alphabet = 2^2n, i.e. 1024
    of 2^40 for [[5,1,3]]), so the measured runtime is dominated by the cost
    of traversing the block structure itself rather than by candidate
    processing.  That is a property of the search, not of the timer, and the
    report prints the block accounting that shows it.
    """
    if _np is None:
        raise RuntimeError("numpy backend requested but numpy is not installed")
    np = _np
    ...
    if plan.row_values is None or spec.rows < 2:
        raise RuntimeError("cyclic engine needs rows >= 2 and product form")
    alphabet = spec.row_alphabet
    if (len(plan.row_values[-2]) != alphabet
            or len(plan.row_values[-1]) != alphabet):
        raise RuntimeError("cyclic engine expects the full row alphabet")

    # The SAME shift table the CyclicConstraint predicate uses.
    constraint = plan.constraint
    SHIFT = getattr(constraint, "_shift_table", None)
    if SHIFT is None:
        SHIFT = [cyclic_shift_row(v, spec.n, constraint.shift)
                 for v in range(alphabet)]
    SHIFT_np = np.asarray(SHIFT, dtype=np.int64)
    ALL_VALS = np.arange(alphabet, dtype=np.int64)
    ...
    red_dtype = (np.uint32 if alphabet * max_sum < (1 << 32) else np.uint64)
    ...
    for outer in outer_iter:
        ...
        # ---------- Stage 1: the outer rows must form a shift chain -------
        ok = True
        for i in range(1, p):
            if outer[i] != SHIFT[outer[i - 1]]:
                ok = False
                break
        if not ok:
            short_circuited += 1
        ...
        # ---------- Stage 2: the inner rows are forced by the chain -------
        if p:
            a_arr = np.array([SHIFT[outer[p - 1]]], dtype=np.int64)
        else:
            a_arr = ALL_VALS          # rows == 2: row 0 free, row 1 forced
        b_arr = SHIFT_np[a_arr]
        n_acc = int(a_arr.size)
        retained += n_acc
        ...
        for r in outer:
            w_outer += POP[r]
        w = POP_np[a_arr] + POP_np[b_arr]
        if w_outer:
            w += acc_dtype(w_outer)
        checksum += int(w.sum(dtype=red_dtype))
        if work > 1:
            av = a_arr.astype(val_dtype)
            bv = b_arr.astype(val_dtype)
            vbuf = np.empty(av.shape, dtype=val_dtype)
            obuf = np.empty(av.shape, dtype=np.uint8)
            for t in range(1, work):
                sel = subsets[(t - 1) % nsub]
                const = 0
                for i in sel:
                    if i < p:
                        const ^= outer[i]
                np.bitwise_xor(av, bv, out=vbuf)
                if const:
                    vbuf ^= val_dtype(const)
                np.take(POP_val, vbuf, out=obuf)
                checksum += int(obuf.sum(dtype=red_dtype))

        # ---------- target-code identification (no early exit) ------------
        if probe is not None:
            ok_t = True
            for i in range(p):
                if outer[i] not in probe.position_values[i]:
                    ok_t = False
                    break
            if ok_t:
                for idx in range(n_acc):
                    ra = int(a_arr[idx])
                    rb = int(b_arr[idx])
                    if ra in tset2 and rb in tset3:
                        cand = outer + (ra, rb)
                        if probe.test(cand):
                            hits += 1
                            if first_hit is None:
                                first_hit = base + ra * alphabet + rb
                                print("Code Found")
                            elif announce_all:
                                print("Code Found")
    ...
    res.pair_stage_evaluated = 0        # inner rows are forced, not tested
    return res


def search_numpy_cyclic_generate(plan: SearchPlan) -> SearchResult:
    """
    CONSTRUCTIVE ENGINE for cyclicity (EXPERIMENT D, --mode generate, DEFAULT).

    This engine does NOT traverse the 2^40 candidate space and test each
    matrix for cyclicity.  Instead it enumerates every possible FIRST ROW --
    all 2^2n of them -- and GENERATES the remaining rows from it:

        row_0 = v                      for every v in the row alphabet
        row_{i+1} = cyclic_shift(row_i, shift)

    Because a cyclic matrix is completely determined by its first row, this
    enumerates the retained subspace exactly once, with no cyclicity test
    performed after generation: every matrix produced is cyclic by
    construction, and every cyclic matrix is produced exactly once.  The
    enumerated count is therefore S_R = 2^2n itself.

    WHAT THIS CHANGES, AND WHY
    --------------------------
    The other three benchmarks (none / universal / weight-4) traverse all
    2^40 candidates and reject the ones that fail.  Their runtime is bounded
    below by the cost of walking the enumeration structure, which for
    cyclicity swamped the answer: the filter-mode measurement of this same
    constraint spent its whole time rejecting 2^20 blocks to keep 1024
    matrices, landing four orders of magnitude above S_R/S_0.

    Generating from the first row removes that floor, so the measured
    runtime scales with S_R and can be compared against the theoretical
    retention directly.  The trade-off is that this is no longer an
    exhaustive-traversal-plus-filter measurement, so it is NOT
    methodologically identical to the other three; state which mode was used
    when reporting it.  --mode filter still runs the traversal version.

    The cyclicity definition, shift table and convention are unchanged --
    the same CyclicConstraint from [S4].  Nothing else is imposed: no rank,
    no commutation, no weight spectrum, no distance, no other condition.
    The processing workload (--work) and timing methodology are identical to
    the other programs.
    """
    if _np is None:
        raise RuntimeError("numpy backend requested but numpy is not installed")
    ...
    if plan.row_values is None:
        raise RuntimeError("generate engine needs the first-row alphabet")
    ...
    POP = plan.pop_table
    POP_np = np.asarray(POP, dtype=acc_dtype)
    POP_val = np.asarray(POP, dtype=np.uint8)
    ...
    probe = plan.target
    if probe is not None:
        flag0 = np.array([v in probe.position_values[0]
                          for v in range(alphabet)], dtype=bool)
    announce_all = plan.announce_all

    first_rows = np.asarray(plan.row_values[0], dtype=np.int64)
    m = int(first_rows.size)
    red_dtype = (np.uint32 if max(1, m) * max_sum < (1 << 32) else np.uint64)
    ...
    # ---------- candidate generation: rows 1.. from the first row ----------
    rows = [first_rows]
    for _ in range(1, spec.rows):
        rows.append(SHIFT_np[rows[-1]])

    # ---------- per-candidate processing (identical workload) --------------
    # unit 0: the sum of the matrix's row weights
    w = POP_np[rows[0]].copy()
    for i in range(1, spec.rows):
        w += POP_np[rows[i]]
    checksum += int(w.sum(dtype=red_dtype))

    # units 1..work-1: weights of row-space combinations, same subsets and
    # same order as every other engine in this set
    if work > 1:
        vbuf = np.empty(m, dtype=val_dtype)
        obuf = np.empty(m, dtype=np.uint8)
        rows_v = [r.astype(val_dtype) for r in rows]
        for t in range(1, work):
            sel = subsets[(t - 1) % nsub]
            np.copyto(vbuf, rows_v[sel[0]])
            for i in sel[1:]:
                vbuf ^= rows_v[i]
            np.take(POP_val, vbuf, out=obuf)
            checksum += int(obuf.sum(dtype=red_dtype))
    ...
    # ---------- target-code identification (no early exit) -----------------
    if probe is not None:
        for idx in np.flatnonzero(flag0[first_rows]):
            cand = tuple(int(rows[i][idx]) for i in range(spec.rows))
            if probe.test(cand):
                hits += 1
                if first_hit is None:
                    first_hit = int(first_rows[idx])
                    print("Code Found")
                elif announce_all:
                    print("Code Found")
    ...
    res.first_target_index = first_hit
    res.blocks_total = 0
    res.blocks_short_circuited = 0
    res.pair_stage_evaluated = 0
    return res
...
def run_search(plan: SearchPlan) -> SearchResult:
    ...
    if plan.backend == "numpy":
        if isinstance(plan.constraint, CyclicConstraint):
            if plan.mode == "generate":
                # DEFAULT: enumerate every first row and generate the rest.
                return search_numpy_cyclic_generate(plan)
            if plan.check_constraint:
                # --mode filter: traverse 2**40 and test each candidate.
                return search_numpy_cyclic(plan)
...
def report(spec: CodeSpec, constraint: Constraint, mode: str, backend: str,
    ...
    if mode == "generate":
        print("Enumeration: CONSTRUCTIVE -- every possible first row is "
              "enumerated and the remaining rows are generated from it; no "
              "cyclicity test is performed after generation, so the "
              "enumerated set IS the retained subspace S_R.")
        print(f"S_R / S_0: {r0.examined / spec.space_size:.12g}")
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--constraint", default="cyclic",
    ...
    p.add_argument("--mode", default="generate", choices=("filter", "generate"),
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    universal = isinstance(constraint, CyclicConstraint)
    if universal and args.mode not in ("generate", "filter"):
        print("--mode must be 'generate' (default: build each matrix from "
              "its first row) or 'filter' (traverse 2**40 and test).",
              file=sys.stderr)
        return 2
