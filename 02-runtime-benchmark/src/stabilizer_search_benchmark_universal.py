...
# -*- coding: utf-8 -*-
"""
stabilizer_search_benchmark_universal.py
========================================

EXPERIMENT (B): UNIVERSAL-STABILIZER-CONSTRAINT RUNTIME BENCHMARK.

Derived from stabilizer_search_benchmark.py (experiment (A), the
unconstrained baseline, preserved verbatim as
stabilizer_search_benchmark_unconstrained_ORIGINAL.py).  The enumeration
architecture, matrix representation, enumeration order, candidate-processing
workload, target recognition and timing methodology are unchanged.  The only
addition is the universal stabilizer constraint test.

THE UNIVERSAL STABILIZER CONSTRAINT (the ONLY thing added)
----------------------------------------------------------
A candidate H = [ H_X | H_Z ] is accepted if and only if BOTH hold:

    (1) rank(H) = n - k      over GF(2)   (the n-k generator rows are
                                           linearly independent)

    (2) H_X H_Z^T = H_Z H_X^T   (mod 2)   (pairwise stabilizer commutation;
                                           equivalently the symplectic form
                                           <r_i, r_j> = 0 for every pair)

Nothing else is imposed.  No distance condition, no [[5,1,3]] validity test,
no weight spectrum, no cyclicity, no automorphism condition, no canonical
form, no removal of duplicate generating sets.

CONSTRAINED-SEARCH PIPELINE (differs from the baseline only here)
-----------------------------------------------------------------
    enumerate candidate
        -> evaluate universal constraint
            -> FAIL: skip immediately (no candidate-processing workload)
            -> PASS: run the SAME candidate-processing workload as (A)

Consequently the processing checksum of this program covers the ACCEPTED
candidates only, while the enumeration still covers exactly 2^40 candidates
(reported as "Total candidate matrices considered").

CONSTRAINT-EVALUATION STRATEGY (--constraint-eval)
--------------------------------------------------
The constraint is a conjunction of pairwise conditions, so it is evaluated
hierarchically: sub-conditions that depend on fewer rows are tested first and
a candidate is rejected by the cheapest sub-condition it actually violates.
Every one of the 2^40 candidates has its constraint status determined; none
is skipped untested.  Two strategies are provided because they measure
different things:

  hierarchical  (default)  Short-circuits on shared row prefixes.  If rows 0
                           and 1 already fail (they do not commute, or they
                           are dependent) then every candidate sharing that
                           prefix is rejected without descending further.
                           This is what a real constrained search costs.

  per-candidate            Materialises the full (2^2n x 2^2n) acceptance
                           mask for every block, so the constraint cost is
                           Theta(2^40) with no prefix amortisation.  This
                           measures the cost of filtering the full space.

Report which one you used; they are not interchangeable.

EXPECTED ACCEPTED COUNT (validation reference, NOT used by the search)
----------------------------------------------------------------------
The number of ordered m-tuples of independent, pairwise-commuting rows in a
2n-dimensional symplectic GF(2) space is

        prod_{i=0}^{m-1} ( 2^(2n-i) - 2^i )

For [[5,1,3]]_2 (n=5, m=4) that is 1023 * 510 * 252 * 120 =
15,777,115,200, i.e. a fraction 0.0143492026837 of S_0 = 2^40.  The search
does NOT use this formula; it is printed by --dry-run purely so a completed
run can be validated against it.

WHY T_universal / T_unconstrained CANNOT REACH S_R / S_0
---------------------------------------------------------
Naively one expects T_universal ~ 0.01435 * T_unconstrained.  It cannot be,
and the reason is structural rather than an implementation defect.

Rejected candidates genuinely do skip the processing workload here.  But the
constraint still has to be EVALUATED, and it must be evaluated on more
candidates than it accepts.  Counting for [[5,1,3]]_2:

  blocks traversed                        1,048,576
  blocks surviving stage 1 (r0,r1 ok)       521,730   (49.76%)
  candidates reaching the pair stage     33,131,941,920   = 3.0133% of S_0
  candidates accepted                    15,777,115,200   = 1.4349% of S_0

So 2.10 candidates have their constraint status resolved at the pair stage
for every 1 candidate accepted.  At the unconstrained run's own throughput
that pair-stage evaluation alone costs roughly 3.0133% of T_unconstrained
before a single accepted candidate is processed.

On top of that, vectorisation efficiency falls: the unconstrained engine
amortises numpy's fixed per-call overhead over 1,048,576 candidates per
block, whereas the constrained engine amortises it over ~30,240 accepted
candidates per block -- 34x fewer.

Both effects are properties of the constrained search itself, not of the
timer, and quantifying the gap between the predicted and observed ratio is
one of the results this experiment exists to produce.

CLOSING THE GAP WITH --work
----------------------------
The first effect is an ADDITIVE fixed cost F (constraint evaluation plus
extraction of the accepted candidates); the processing cost is proportional
to S_R.  So
...
and therefore
        ...
        T_R/T_0  ->  (c_R/c_0) * (S_R/S_0)      as w -> infinity.

--work w raises the per-candidate processing until F is negligible.  Both
programs must be run with the SAME w; w=1 is the original workload.

Two things to expect, both measured rather than assumed:

  * F is roughly 90 s worth of work on the reference machine, so w must be
    several units before it stops dominating -- and each unit multiplies the
    UNCONSTRAINED runtime by a large factor, since it applies to all 2^40
    candidates.  Reaching the regime where the two agree costs hours of
    baseline runtime.  Budget for it, or fit the model above from three or
    four cheap w values instead of chasing a single matching number.

  * c_R/c_0 < 1.  The constrained engine processes ~30,240 candidates per
    block (a working set of order 100 KB, cache resident) while the
    unconstrained engine processes 1,048,576 (several MB, not resident).
    Identical arithmetic, different memory locality.  The observed ratio
    therefore crosses S_R/S_0 and keeps falling instead of settling on it.

IMPLEMENTATION NOTE (measured, 2026-08-22)
-------------------------------------------
An earlier version of this engine extracted the accepted candidates into a
compact list (np.nonzero + int64 index gathers) so that arithmetic touched
accepted candidates only.  Profiling showed that machinery cost MORE than
the arithmetic it avoided:

    COMM[np.ix_(S, S)]   2D fancy gather      ~134 us/block
    np.nonzero(sub)      -> int64 index pairs ~167 us/block
    sub.sum()  on bool                         ~31 us/block
    np.count_nonzero(sub) on bool               ~2 us/block

The current engine therefore uses a contiguous row-then-column gather
(COMM[S][:, S], ~43 us/block), np.count_nonzero, preallocated output
buffers, a uint32 reduction accumulator, and a dense masked reduction over
the surviving sub-block.  Measured ~3.3x faster end to end with bit-identical
examined / retained / checksum / target-hit results.
...
def search_numpy_universal(plan: SearchPlan) -> SearchResult:
    """
    CONSTRAINED ENGINE for the universal stabilizer constraints.

    Same blocked traversal as search_numpy(): the first (rows-2) row
    positions are iterated in Python, the final two form an
    (alphabet x alphabet) block, and the enumeration order is the identical
    lexicographic order (row_0 slowest, row_{rows-1} fastest).  `examined`
    still advances by the full block size for every block, so the traversal
    still covers exactly S_0 candidates.

    The universal constraint

        (1) rank(H) = rows            (2) H_X H_Z^T = H_Z H_X^T  (mod 2)

    is a conjunction of pairwise conditions, so it is evaluated in three
    stages ordered by how many rows each sub-condition touches.  A candidate
    is rejected by the cheapest sub-condition it actually violates, and only
    survivors reach the candidate-processing workload.

      Stage 1 (outer rows only)   outer rows must pairwise commute and be
                                  independent.  If not, every candidate
                                  sharing that prefix violates the
                                  constraint.  In "hierarchical" mode the
                                  block is rejected here; in "per-candidate"
                                  mode the full block mask is still
                                  materialised so that the constraint cost
                                  stays Theta(S_0).
      Stage 2 (one inner row)     a row value may appear at an inner
                                  position only if it commutes with every
                                  outer row and lies outside their span.
                                  Yields the surviving value set S.
      Stage 3 (the inner pair)    the two inner rows must commute with each
                                  other (table lookup on S x S) and must not
                                  be dependent, i.e. r3 must avoid
                                  { r2 ^ u : u in span(outer) }.

    Everything precomputed (the commutation table, the popcount table) is a
    table over ROW VALUES, built outside the measured interval, exactly as in
    the baseline.
    """
    if _np is None:
        raise RuntimeError("numpy backend requested but numpy is not installed")
    ...
    COMM = getattr(constraint, "_comm", None)
    if COMM is None:
        raise RuntimeError("commutation table unavailable; use --backend pure")
    if plan.row_values is None or spec.rows < 2:
        raise RuntimeError("universal engine needs rows >= 2 and product form")

    res = SearchResult()
    alphabet = spec.row_alphabet
    outer_values = plan.row_values[:-2]
    p = spec.rows - 2                      # number of Python-iterated rows
    block = alphabet * alphabet
    per_candidate = (plan.constraint_eval == "per-candidate")

    # The block is the full value-by-value grid, so a candidate's index
    # inside its block is simply r_{rows-2} * alphabet + r_{rows-1}.
    if (len(plan.row_values[-2]) != alphabet
            or len(plan.row_values[-1]) != alphabet):
        raise RuntimeError("universal engine expects the full row alphabet")

    max_sum = spec.rows * spec.cols
    acc_dtype = (np.uint8 if max_sum < 256
                 else np.uint16 if max_sum < 65536 else np.uint32)
    # Widening accumulator for the per-block reduction. uint32 is measurably
    # faster than uint64 and cannot overflow while block * max_sum fits.
    red_dtype = np.uint32 if block * max_sum < (1 << 32) else np.uint64

    POP = plan.pop_table                              # plain list: fast scalars
    POP_np = np.asarray(POP, dtype=acc_dtype)

    # ---- buffers allocated once, OUTSIDE the traversal -------------------
    mbuf = np.empty(alphabet, dtype=bool)             # stage-2 row mask
    pos = np.full(alphabet, -1, dtype=np.int32)       # row value -> index in S
    ar32 = np.arange(alphabet, dtype=np.int32)
    ridx_col = np.arange(alphabet).reshape(alphabet, 1)
    Vbuf = np.empty(1 << p, dtype=np.int64)           # span(outer)
    ALL_VALS = np.arange(alphabet, dtype=np.int64)    # value axis, per-cand.
    fullbuf = (np.empty((alphabet, alphabet), dtype=bool)
    ...
    # Target gates, precomputed per row position (outside the traversal).
    probe = plan.target
    if probe is not None:
        tset2 = probe.position_values[spec.rows - 2]
        tset3 = probe.position_values[spec.rows - 1]
    announce_all = plan.announce_all
    ...
    for outer in outer_iter:
        ...
        # ---------- Stage 1: conditions among the outer rows only ----------
        # Commutation and independence are checked together while the span
        # span(outer) is built, so no separate rank elimination is needed:
        # rank(outer) = p  <=>  no outer row lies in the span of its
        # predecessors.  Both are O(1) table/set operations per row pair.
        outer_ok = True
        nv = 1
        Vbuf[0] = 0
        for i in range(p):
            ri = outer[i]
            for j in range(i):
                if not COMM[ri, outer[j]]:           # (2) commutation
                    outer_ok = False
                    break
            if not outer_ok:
                break
        ...
        if per_candidate:
            # ---- full-block evaluation: no prefix amortisation at all ----
            if outer_ok:
                if p:
                    np.bitwise_and(COMM[outer[0]], COMM[outer[1]], out=mbuf) \
                        if p >= 2 else np.copyto(mbuf, COMM[outer[0]])
                    for i in range(2, p):
                        mbuf &= COMM[outer[i]]
                else:
                    mbuf.fill(True)
                mbuf[Vbuf[:nv]] = False
                np.copyto(fullbuf, COMM)              # (2) inner pair
                fullbuf &= mbuf[:, None]              # (1)+(2) inner row A
                fullbuf &= mbuf[None, :]              # (1)+(2) inner row B
                for t in range(nv):                   # (1) inner pair depend.
                    fullbuf[ridx_col, ridx_col ^ int(Vbuf[t])] = False
            else:
                fullbuf.fill(False)
            mask = fullbuf
            n_acc = int(np.count_nonzero(mask))
            S = None                                  # values are the indices
            ns = alphabet
        else:
            # ---- hierarchical evaluation (default) ----
            if not outer_ok:
                short_circuited += 1
                if plan.progress and blocks_done % prog_every == 0:
                    _progress_line(blocks_done, outer_total, t0)
            ...
            # -- Stage 2: conditions involving one inner row --
            if p >= 2:
                np.bitwise_and(COMM[outer[0]], COMM[outer[1]], out=mbuf)
                for i in range(2, p):
                    mbuf &= COMM[outer[i]]
            elif p == 1:
                np.copyto(mbuf, COMM[outer[0]])
            else:
                mbuf.fill(True)
            mbuf[Vbuf[:nv]] = False           # inner row outside span(outer)
            S = np.flatnonzero(mbuf)
            ns = int(S.size)
            if ns == 0:
                if plan.progress and blocks_done % prog_every == 0:
                    _progress_line(blocks_done, outer_total, t0)
            ...
            # -- Stage 3: the inner pair --
            # COMM[S][:, S] is a contiguous row gather followed by a column
            # gather; measured ~3x faster than COMM[np.ix_(S, S)].
            mask = COMM[S][:, S]                      # (2) inner-pair commut.
            pos[S] = ar32[:ns]
            # (1) inner pair dependence: r_last must avoid r_prev ^ span(outer)
            mask[ridx_col[:ns], pos[S[:, None] ^ Vbuf[None, :nv]]] = False
            n_acc = int(np.count_nonzero(mask))       # count_nonzero >> .sum()
            pair_stage += ns * ns

        retained += n_acc

        # ---------- candidate processing: ACCEPTED candidates only ---------
        # The accepted candidates are first EXTRACTED into a compact array,
        # so every operation below runs exactly n_acc times: a rejected
        # candidate consumes no processing time whatsoever.  Extraction via
        # flatnonzero measured ~2.2x cheaper than np.nonzero.  Candidates
        # rejected at stage 1 or stage 2 -- the overwhelming majority -- never
        # reach this code at all.
        #
        # This is what makes the runtime scale with S_R rather than with the
        # number of candidates tested; the arithmetic itself is identical to
        # the unconstrained baseline's, unit for unit.
        if n_acc:
            w_outer = 0
            for r in outer:
                w_outer += POP[r]
            axis_vals = ALL_VALS if per_candidate else S
            nrows_m, ncols_m = mask.shape
            flat = np.flatnonzero(mask.ravel())

            # Fast path: every mask row holds the same number of accepted
            # candidates (it does for this constraint), so the flat index
            # list reshapes directly. Verified per block by checking that the
            # first and last entry of each group really lie in that row; if
            # the check ever fails we fall back to explicit division.
            fast = False
            if n_acc % nrows_m == 0:
                grp = flat.reshape(nrows_m, n_acc // nrows_m)
                rows_ix = np.arange(nrows_m)
                fast = (np.array_equal(grp[:, 0] // ncols_m, rows_ix)
                        and np.array_equal(grp[:, -1] // ncols_m, rows_ix))
            if fast:
                jj = grp - (rows_ix * ncols_m).reshape(nrows_m, 1)
                avals = axis_vals.reshape(nrows_m, 1)      # broadcasts
                bvals = axis_vals[jj]
            else:
                ii = flat // ncols_m
                jj = flat - ii * ncols_m
            ...
            # unit 0: the original workload -- sum of the matrix's row weights
            w = POP_np[avals] + POP_np[bvals]
            if w_outer:
                w += acc_dtype(w_outer)
            checksum += int(w.sum(dtype=red_dtype))

            # units 1..work-1: weights of row-space combinations. Same
            # operations, in the same order, as the unconstrained engine.
            if work > 1:
                av = np.broadcast_to(avals, bvals.shape).astype(val_dtype)
                bv = bvals.astype(val_dtype)
                vbuf_c = np.empty(bv.shape, dtype=val_dtype)
                obuf_c = np.empty(bv.shape, dtype=np.uint8)
                for t in range(1, work):
                    sel = subsets[(t - 1) % nsub]
                    const = 0
                    for i in sel:
                        if i < p:
                            const ^= outer[i]
                    np.bitwise_xor(av, bv, out=vbuf_c)
                    if const:
                        vbuf_c ^= val_dtype(const)
                    np.take(POP_val, vbuf_c, out=obuf_c)
                    checksum += int(obuf_c.sum(dtype=red_dtype))

            # ---------- target-code identification (no early exit) --------
            # Gated so it runs only for blocks whose outer rows already lie
            # in the target span; at most a few hundred blocks in total.
            if probe is not None:
                ok_t = True
                for i, r in enumerate(outer):
                ...
                if ok_t:
                    if S is None:
                        q2 = sorted(v for v in tset2 if v < alphabet)
                        q3 = sorted(v for v in tset3 if v < alphabet)
                        idx2 = [(v, v) for v in q2]
                        idx3 = [(v, v) for v in q3]
                    else:
                        idx2 = [(q, int(S[q])) for q in range(ns)
                                if int(S[q]) in tset2]
                        idx3 = [(q, int(S[q])) for q in range(ns)
                                if int(S[q]) in tset3]
                    for q2i, ra in idx2:              # ascending == lex order
    ...
    res.pair_stage_evaluated = pair_stage if not per_candidate else examined
    return res
...
def run_search(plan: SearchPlan) -> SearchResult:
    ...
    if plan.backend == "numpy":
        if (isinstance(plan.constraint, UniversalStabilizerConstraint)
                and plan.check_constraint):
            return search_numpy_universal(plan)
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    p.add_argument("--constraint", default="universal",
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    universal = isinstance(constraint, UniversalStabilizerConstraint)
    if universal and args.mode != "filter":
        print("The universal constraint couples rows, so it has no "
              "constructive row-local form; use --mode filter.",
              file=sys.stderr)
        return 2
    ...
    if universal:
        print(f"Constraint: rank(H)={spec.rows} AND H_X H_Z^T = H_Z H_X^T "
              f"(mod 2); evaluation={args.constraint_eval}", file=sys.stderr)
